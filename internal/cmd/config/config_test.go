...
func TestDatabaseMaxIdleConnections(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expErr: true,
			expErrStr: "Database max idle connections value is not an int: " +
				"strconv.Atoi: parsing \"string bad\": invalid syntax",
		},
		{
			name: "Invalid value type",
...
func TestDatabaseConnMaxIdleTimeDuration(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expErr:    true,
			expErrStr: "strconv.ParseInt: parsing \"string ba\": invalid syntax",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("ENV_MAX_IDLE_TIME", tt.envConnMaxIdleTimeDuration)
			c, err := Parse(tt.in)
			if tt.expErr {
				require.EqualError(t, err, tt.expErrStr)
				require.Nil(t, c)
